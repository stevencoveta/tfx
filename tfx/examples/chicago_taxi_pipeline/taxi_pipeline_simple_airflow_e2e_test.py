...
class AirflowEndToEndTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self._database_name = 'airflow_' + test_utils.generate_random_id()
    airflow_test_utils.create_mysql_database(self._database_name)
    self.addCleanup(airflow_test_utils.drop_mysql_database, self._database_name)
    ...
    os.environ['AIRFLOW__CORE__SQL_ALCHEMY_CONN'] = (
        'mysql://tfx@127.0.0.1:3307/' + self._database_name)
